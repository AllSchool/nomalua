...
function NOMALUA.AddLuaFiles( luaFilesTable, dir, recurse, root )
	...
	for i=1, #result do
		if (string.Right(result[ i ],4) == ".lua") then
			...
			-- - filtrer tous les addons/... fichiers que nous avons déjà inclus dans lua/.....
			-- - par exemple, addons/myaddon/lua/file.lua et lua/file.lua apparaîtront tous deux.
			-- - supprimer lua/file.lua (remplacer la clé par la valeur addons/...)
